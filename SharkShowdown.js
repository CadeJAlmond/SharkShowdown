...
graphOptions = {
    ...
    scales: {
        y: {
            stacked: true,
            title: {
                display: true, text: "Population Size", color: "#eaeaea", font: {
                    size: 28,
        ...
        x: {
            title: {
                display: true, text: "Years", color: "#eaeaea", font: {
                    size: 28,
                    family: "Source Sans Pro",
                }, padding: { top: 15 }
            },
...
function beginSimulation(iterations, sharks, minnows) {
    ...
    updatePageGraph(sharkDataSet, minnowDataSet, labels, graphOptions);
    updatePageStats(iterations, wentExtinct);
}
...
function updatePopulations() {
    ...
    if (changeInMinnows > minnowPopulation)
        changeInMinnows = -minnowPopulation;
    if (changeInSharks > sharkPopulation)
...
function recordPopulations(iterations) {
    minnowDataSet = [...minnowDataSet, minnowPopulation];
    sharkDataSet = [...sharkDataSet, sharkPopulation];
    labels = [...labels, iterations + 1];
}
 ...
 */
function updatePageGraph(sharkData, minnowData, labelData, options) {
 ...
 * of the simulation
 */
function updatePageStats(iterations, wentExtinct) {
...
// Load graph on-website load
updatePageGraph([], [], [], { scales: graphOptions.scales });
